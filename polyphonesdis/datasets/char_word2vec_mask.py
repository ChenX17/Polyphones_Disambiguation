...
from polyphonesdis.datasets.utils import load_vocab, build_tags_from_file, gen_mask_seq, gen_mask, find_word, load_poly_lexicon


class CHARW2CPOSCWSFLAGMASKDataSet(torch.utils.data.Dataset):
    """CHAR&WORD2VEC dataset"""
    ...
    def __getitem__(self, index):
        ...
        mask = gen_mask_seq(texts, self.tag_to_index)
    ...
    def pad_and_sort_by_len(self, batch):
        ...
        mask_seq = np.zeros((len(sorted_batch), sorted_batch[0][1].shape[0], len(self.tag_to_index))) 
        for i,x_y in enumerate(sorted_batch):
            if seq_lens[i] != x_y[1].shape[0]:
                ...
                mask_seq[i, :seq_lens[i], :]=x_y[3]
        word2vecs = torch.tensor(word2vecs, dtype=torch.float16,)
        mask = torch.tensor(mask_seq, dtype=torch.float16)
