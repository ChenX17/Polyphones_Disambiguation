...
class CHARW2VFLAGNet(nn.Module):
    ...
    def __init__(self,
        ...
        super(CHARW2VFLAGNet, self).__init__()
        self.device = device
        ...
        self.bilstm = nn.LSTM(input_size=flag_size + self.embedding_dim + self.pretrained_embedding_dim,
    ...
    def forward(self, inputs, target, seq_lens):
        ...
        #FLAG_features = self.FLAG_embeds(inputs['FLAG'])
        #FLAG_features = self.dropout(self.layernorm(FLAG_features))
        flag_features =  torch.unsqueeze(inputs['flag'], dim=-1)

        input_features = torch.cat((input_features, embedding_inputs.float(), flag_features), 2)
